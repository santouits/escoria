...
# The name the .esc script needs to use the entity. It's mandatory.
var global_id

# Every object has some area in order to interact with it
var area
...
enum ActivationType {Click = 0, Collision = 1, None = 2}	
var activation_on = ActivationType.Click
var terrain
var walk_path
var walk_context
var moved
	...
	if body is esc_type.PLAYER:
		if entity.visible:
			...
			if event in event_table:
				vm.run_event(event_table[event])
func stopped_at(pos):
	# Check if the player stopped inside some area
	# If area is a node that inherits Control we use get_global_rect()
	# If area is a node that inherits CollissionObject2D we use some magic
	print("stopped")
	var is_inside = false
	if area is Control and area.get_global_rect().has_point(pos):
		is_inside = true
	elif area is Area2D:
	...
	if is_inside:
		if entity.visible:
			if stopped_action != "" and stopped_action in event_table:
				vm.run_event(event_table[stopped_action])

func init():
	print("init " + entity.name)
	...
	if activation_on == Collision:
		...
		if entity.get("stopped_action"):stopped_action = entity.stopped_action
		if entity.get("enter_action"):enter_action = entity.enter_action
		if entity.get("exit_action"):exit_action = entity.exit_action
		add_to_group("exit")
	...
	if interact_position:
		...
		interact_pos = entity.get_node("interact_pos")
